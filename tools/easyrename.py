...
class Window(tk.Tk):
	...
	def rename_image(self):
		val = self.autocomplete.get()

		if val in self.itemdict:
			if not os.path.exists(self.newpath):
				os.mkdir(self.newpath)

			src = os.path.join(self.oldpath, self.images[self.index])
			dst = os.path.join(self.newpath, self.itemdict[val]+".png")
			...
			print(f'Could not recognize value "{val}"')
