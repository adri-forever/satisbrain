...
class ScrollFrame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.canvas = tk.Canvas(self, borderwidth=0, background="#333")
        # place a frame on the canvas, this frame will hold the child widgets
        self.innerframe = tk.Frame(self.canvas, background="#333")
        ...
        # bind an event whenever the size of the innerframe frame changes.
...
class Window(tk.Tk):
    ...
    def setup_style(self):
        ...
        self.style.configure('TNotebook', relief='flat',
        ...
        self.style.configure('TLabel', relief='flat', background='#333',
                             foreground='white', font=(font, fontsize))
        self.style.configure('high.TLabel', relief='flat', background='#555',
                             foreground='white', font=(font, fontsize+2, 'bold'))
...
class TopButtonBar(ttk.Frame):
    ...
    def __init__(self, master: Window):
        ...
        self.btn_tooltip = tktooltip.ToolTip(
            self.new_btn, msg='Create new plan', delay=.5)
...
class ComputingFrame(ttk.Frame):
    ...
    def __init__(self, master: ttk.Notebook):
        ...
        self.genrep_btn = ttk.Button(
            self.header, text='Generate report', command=self.generate_report)
    ...
    def submit_item(self, target_item: str, qty: float):
        ...
        if self.target_item and target_item != self.target_item:
            confirm = messagebox.askokcancel(
                'Base item change', 'This action will reset the plan. All choices will be lost. Continue ?')
    ...
    def manage_base_resources(self, item: str, action: Literal['add', 'remove'] = 'add'):
        ...
        if action == 'add' and item not in self.base_resources:
            ...
            print(
                f'Could not interpret action "{action}". Instruction ignored')
    ...
    def generate_report(self):
        ...
        path = filedialog.asksaveasfilename(
            filetypes=filetypes, initialdir=initialdir, initialfile=initialfile, title=title)
...
class ItemPicker(ttk.Frame):
    ...
    def __init__(self, master, computingframe: ComputingFrame):
        ...
        self.itemdict = {item_data[0]['name']: item for item, item_data in brain.data.data_items.items(
        ) if item not in self.computingframe.base_resources}
        ...
        vcmd = (self.register(self.qty_validate),
                '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        self.qty_box = ttk.Entry(self, validate='key',
                                 validatecommand=vcmd, width=5)
...
class BaseResourceManager(ttk.Frame):
    ...
    def __init__(self, master, computingframe: ComputingFrame):
        ...
        self.remove_btn = ttk.Button(
            self, text='Remove', command=self.remove_resource)
        self.remove_btn.grid(row=1, column=1, sticky='w')

    def update_tree(self):
        self.itemdict = dict(sorted(
            {brain.data.data_items[item][0]['name']: item for item in self.computingframe.base_resources}.items()))
...
class RecipePicker(ttk.Frame):
    ...
    def __init__(self, master, computingframe, item: str):
        ...
        self.recipedict = {brain.data.data_recipes[recipe][0]['name']
            : recipe for recipe in brain.data.data_itemtorecipes[item]}
        ...
        self.item_box = ttk.Label(
            self.header, text=brain.data.data_items[item][0]['name'], style='high.TLabel')
        self.item_box.pack(side='left', **pads)
        ...
        for i, product in enumerate(recipe_data['products']):
            prodnamelabel = ttk.Label(
                body, text=brain.data.data_items[product['item']][0]['name'])
        ...
        for i, ingredient in enumerate(recipe_data['ingredients']):
            ingrnamelabel = ttk.Label(
                body, text=brain.data.data_items[ingredient['item']][0]['name'])
    ...
    def change_recipe(self, event: tk.Event = None):
        ...
        if boxchoice == 'Set as input':
            ...
            self.computingframe.set_item_recipe(
                self.item, self.recipedict[self.recipe_box.get()])
        else:
